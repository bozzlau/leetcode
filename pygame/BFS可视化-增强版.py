...
class MazeVisualizer:
    def __init__(self, config: Config = None):
        ...
        pygame.display.set_caption("🔍 BFS 迷宫可视化 - 增强版")
    ...
    def bfs(self) -> Generator:
        ...
        step += 1
        visit_order[self.start] = step
        
        # 访问起点
        yield ("visit", self.start, set(visited), dict(visit_order), dict(parent))
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if 0 <= nx < self.config.N and 0 <= ny < self.config.N:
                    if self.maze[nx][ny] == 0 and (nx, ny) not in visited:
                        ...
                        # 每访问一个新节点就 yield
                        yield ("visit", (nx, ny), set(visited), dict(visit_order), dict(parent))
